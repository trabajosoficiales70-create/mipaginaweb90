  ...
  if(cards && cards.length){
    for(let i=0;i<cards.length;i++){
      cards[i].addEventListener("mousemove", function(e){
        ...
        cards[i].style.transform = "rotateY(" + ((x - rect.width/2)/10) + "deg) rotateX(" + (-(y - rect.height/2)/10) + "deg)";
      });
  ...
  if(images && images.length){
    for(let j=0;j<images.length;j++){
      images[j].addEventListener("mouseenter", function(){
        images[j].style.transform = "scale(1.05)";
        images[j].style.boxShadow = "0 0 20px #00ffea";
      });
